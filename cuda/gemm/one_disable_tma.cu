...
extern "C" __global__ void gemm_ws_kernel(__grid_constant__ const CUtensorMap A_desc, const half_t* __restrict__ B, half_t* __restrict__ C);
extern "C" __global__ void __launch_bounds__(256, 1) gemm_ws_kernel(__grid_constant__ const CUtensorMap A_desc, const half_t* __restrict__ B, half_t* __restrict__ C) {
  ...
  if (128 <= ((int)threadIdx.x)) {
    ...
    for (int ko_1 = 0; ko_1 < 32; ++ko_1) {
      ...
      for (int i_1 = 0; i_1 < 4; ++i_1) {
        *(uint4*)(((half_t*)buf_dyn_shmem) + (((((((((ko_1 & 1) * 4096) + (((((int)threadIdx.x) & 15) >> 3) * 2048)) + (i_1 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) + 8192)) = *(uint4*)(B + (((((ko_1 * 32768) + (i_1 * 8192)) + ((((int)threadIdx.x) >> 4) * 1024)) + (((int)blockIdx.x) * 128)) + ((((int)threadIdx.x) & 15) * 8)));
      }
