...
extern "C" __global__ void gemm_ws_kernel(__attribute__((grid_constant)) const MUtensorDescriptor A_desc_0, __attribute__((grid_constant)) const MUtensorDescriptor B_desc_1, __attribute__((grid_constant)) const MUtensorDescriptor C_desc_2);
extern "C" __global__ void __launch_bounds__(256, 1) gemm_ws_kernel(__attribute__((grid_constant)) const MUtensorDescriptor A_desc_0, __attribute__((grid_constant)) const MUtensorDescriptor B_desc_1, __attribute__((grid_constant)) const MUtensorDescriptor C_desc_2) {
  ...
  if ((128 <= ((int)threadIdx.x))) {
    for (int ko = 0; ko < 32; ++ko) {
      ...
      if (tl::tl_shuffle_elect<128>()) {
        __musa_async_add_trans(((ko & 1) + 1), 8192);
        ...
        tl::tma_load<SmemSwizzleGranularity::B32, SmemSwizzleStride::B256, SmemSwizzleLine::B256, CacheHint::CACHE_NORMAL, CacheHint::CACHE_NORMAL>(B_desc_1, ((ko & 1) + 1), (&(((half_t*)buf_dyn_shmem)[((ko & 1) * 4096)])), (((int)blockIdx.x) * 128), (ko * 32), 128, 32);
      }
    }
  }
  if ((((int)threadIdx.x) < 128)) {
    ...
    for (int i_1 = 0; i_1 < 128; ++i_1) {
      ((half_t*)buf_dyn_shmem)[(((((i_1 >> 4) * 2048) + ((((int)threadIdx.x) >> 3) * 128)) + ((i_1 & 15) * 8)) + (((int)threadIdx.x) & 7))] = ((half_t)C_local[i_1]);
    }
    ...
    if (tl::tl_shuffle_elect<128>()) {
      tl::tma_store<SmemSwizzleGranularity::NONE, SmemSwizzleStride::B256, SmemSwizzleLine::B256, CacheHint::CACHE_NORMAL, CacheHint::CACHE_NORMAL>(C_desc_2, (&(((half_t*)buf_dyn_shmem)[0])), (((int)blockIdx.x) * 128), (((int)blockIdx.y) * 128), 128, 128);
      tl::tma_store_arrive();
      tl::tma_store_wait<0>();
    }
  }
