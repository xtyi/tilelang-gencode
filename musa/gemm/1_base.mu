...
extern "C" __global__ void gemm_ws_kernel(__attribute__((grid_constant)) const MUtensorDescriptor A_desc_0, __attribute__((grid_constant)) const MUtensorDescriptor B_desc_1, half_t* __restrict__ C);
extern "C" __global__ void __launch_bounds__(256, 1) gemm_ws_kernel(__attribute__((grid_constant)) const MUtensorDescriptor A_desc_0, __attribute__((grid_constant)) const MUtensorDescriptor B_desc_1, half_t* __restrict__ C) {
  __musa_async_bar_record(4);
  ...
  if ((((int)threadIdx.x) < 128)) {
    ...
    for (int i_1 = 0; i_1 < 128; ++i_1) {
      C[((((((((int)blockIdx.y) * 131072) + ((i_1 >> 4) * 16384)) + ((((int)threadIdx.x) >> 3) * 1024)) + (((int)blockIdx.x) * 128)) + ((i_1 & 15) * 8)) + (((int)threadIdx.x) & 7))] = ((half_t)C_local[i_1]);
    }
  }
}
