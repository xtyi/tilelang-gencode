  ...
  if (128 <= ((int)threadIdx.x)) {
    ...
    for (int k = 0; k < 32; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(A_desc, mbarrier[0], (&(((half_t*)buf_dyn_shmem)[0])), (k * 32), (((int)blockIdx.y) * 128));
        mbarrier[0].expect_transaction(8192);
        tl::fence_proxy_async();
        tl::tma_load(B_desc, mbarrier[0], (&(((half_t*)buf_dyn_shmem)[4096])), (((int)blockIdx.x) * 128), (k * 32));
        tl::tma_load(B_desc, mbarrier[0], (&(((half_t*)buf_dyn_shmem)[6144])), ((((int)blockIdx.x) * 128) + 64), (k * 32));
      }
      mbarrier[0].arrive();
    }
  } else {
