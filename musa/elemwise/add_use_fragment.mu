...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(float* __restrict__ A, float* __restrict__ B, float* __restrict__ C) {
  ...
  for (int i = 0; i < 2; ++i) {
    float4 __1;
      float4 v_ = *(float4*)(A + (((((int)blockIdx.x) * 2048) + (i * 1024)) + (((int)threadIdx.x) * 4)));
      float4 v__1 = *(float4*)(B + (((((int)blockIdx.x) * 2048) + (i * 1024)) + (((int)threadIdx.x) * 4)));
      __1.x = (v_.x+v__1.x);
      __1.y = (v_.y+v__1.y);
      __1.z = (v_.z+v__1.z);
      __1.w = (v_.w+v__1.w);
  ...
  for (int i_1 = 0; i_1 < 2; ++i_1) {
    *(float4*)(C + (((((int)blockIdx.x) * 2048) + (i_1 * 1024)) + (((int)threadIdx.x) * 4))) = *(float4*)(sum + (i_1 * 4));
  }
}
