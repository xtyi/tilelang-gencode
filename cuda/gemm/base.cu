...
extern "C" __global__ void gemm_ws_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, half_t* __restrict__ C);
extern "C" __global__ void __launch_bounds__(256, 1) gemm_ws_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, half_t* __restrict__ C) {
  ...
  if (128 <= ((int)threadIdx.x)) {
    ...
    for (int i_2 = 0; i_2 < 64; ++i_2) {
      ...
      *(uint1*)(C + ((((((((((int)blockIdx.y) * 131072) + ((i_2 >> 5) * 65536)) + ((((int)threadIdx.x) >> 5) * 16384)) + ((i_2 & 1) * 8192)) + (((((int)threadIdx.x) & 31) >> 2) * 1024)) + (((int)blockIdx.x) * 128)) + (((i_2 & 31) >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(C_local_cast + 0);
    }
  }
}
